...
namespace _200oker {
    public class Checker {
        ...
        public void PerformCheck (string url, string childSelector, bool dupeCheck = false, string header = "200OKer (https://github.com/carsales/200oker-core)") {
            ...
            try {
                ...
                if (ex.Response != null) {
                    ...
                    Console.WriteLine ("Could Not Get Response:" + ex.Status + " - " + ex.Message);
                }
            }
        }

        private void SetResultFromResponse (string url, HttpWebResponse resp) {
            Console.WriteLine ("URL: {0} - Status: {1} - Description: {2}", url, resp.StatusCode,
        ...
        public void CheckChildren (string parentUrl, string childSelector, string html) {
            ...
            dom.LoadHtml (html);
            var links = dom.DocumentNode.SelectNodes (childSelector);
            if (links == null) {
                Console.WriteLine ("No URLs to check for {0} selector", childSelector);
                return;
            }
            ...
            Parallel.ForEach (links, new ParallelOptions () { MaxDegreeOfParallelism = Config.MaxChildThreads }, o => {
                ...
                if (o.Attributes["href"] != null) {
                    url = o.Attributes["href"].Value;
                }
                // ignore empty
